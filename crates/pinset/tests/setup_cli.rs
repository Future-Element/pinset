...
fn cli(root: &Path, home: &Path, args: &[&str]) -> Output {
    Command::new(env!("CARGO_BIN_EXE_pinset")).current_dir(root).env("PINSET_HOME", home)
        .env_remove("PINSET_ENV_PROFILE").env_remove("PINSET_ENV_DISABLE").args(args).output().unwrap()
}
...
fn offline_failure_is_resumable_but_changed_configuration_is_rejected() {
    ...
    let output = cli(root.path(), &home, &["setup", "--yes", "--offline", "--json"]);
    assert_eq!(output.status.code(), Some(1));
    let value: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
    let id = value["data"]["run"]["id"].as_str().unwrap();
    assert_eq!(value["data"]["run"]["plan"]["steps"][0]["state"], "failed");
    fs::write(&config, "schema = 5\n[tools]\nnode = \"22.0.0\"\n").unwrap();
    let resumed = cli(root.path(), &home, &["setup", "--resume", id, "--yes", "--offline", "--json"]);
...
fn reports_do_not_claim_execution_or_expose_local_paths() {
    ...
    fs::write(root.path().join("pinset.toml"), "schema = 5\n[tools]\nnode = \"24.0.0\"\n").unwrap();
    let output = cli(root.path(), &home, &["status", "--report-version", "2", "--json"]);
    assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
    let value: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
...
fn protocol_two_is_opt_in_and_preserves_protocol_one() {
    ...
    let home = root.path().join("home");
    for protocol in ["1", "2"] {
        let output = cli(root.path(), &home, &["editor", "context", "--protocol", protocol, "--json"]);
        assert!(output.status.success(), "{}", String::from_utf8_lossy(&output.stderr));
        let value: serde_json::Value = serde_json::from_slice(&output.stdout).unwrap();
        assert_eq!(value["data"]["protocol_schema"].as_u64().unwrap(), protocol.parse::<u64>().unwrap());
