...
use std::{env, fs, io::ErrorKind, path::{Path, PathBuf}};
...
pub fn environment_selection(
    ...
    let ci = ["CI", "GITHUB_ACTIONS", "GITLAB_CI", "TF_BUILD"]
        .iter()
        .any(|name| env::var(name).is_ok_and(|value| {
            !value.is_empty() && value != "0" && !value.eq_ignore_ascii_case("false")
        }));
    select_environment(home, config_path, config, explicit, process_profile.as_deref(), ci)
}
...
pub fn select_environment(
    ...
    } else if let Some(value) = if ci { None } else { load_local_environment(home, config_path, config)? } {
        (Some(value), "local")
    } else {
        (config.environment.as_ref().and_then(|value| value.auto_profile.clone()), "project")
    };
    if let Some(profile) = &profile {
        if !config.environment.as_ref().is_some_and(|value| value.profiles.contains_key(profile)) {
            return invalid(config_path, format!("profile {profile:?} is not declared; select an existing profile or run `pinset env use --reset`"));
        }
    }
    Ok(EnvironmentSelection { source: if profile.is_some() { source } else { "none" }, profile })
}
...
fn identity(config_path: &Path) -> Result<String> {
    ...
    Ok(if cfg!(windows) { value.to_ascii_lowercase() } else { value })
}
...
fn state_path(home: &Path, root: &str) -> PathBuf {
    ...
    home.join("state").join("environments").join(format!("{name}.toml"))
}

fn validate_directory(path: &Path) -> Result<()> {
    match fs::symlink_metadata(path) {
        Ok(metadata) if metadata.file_type().is_symlink() || !metadata.is_dir() => invalid(path, "expected a regular directory"),
        Ok(_) => Ok(()),
...
fn load_local_environment(home: &Path, config_path: &Path, config: &ProjectConfig) -> Result<Option<String>> {
    ...
    let record: LocalEnvironment = toml::from_str(&content).map_err(|_| Error::LocalEnvironment {
        path: path.clone(), reason: "invalid state file; run `pinset env use --reset`".to_owned(),
    })?;
    if record.schema != 1 || record.root != root || Some(record.project_id.as_str()) != config.project_id.as_deref() {
        return invalid(&path, "project identity changed; run `pinset env use --reset`");
    }
...
pub fn save_local_environment(home: &Path, config_path: &Path, profile: Option<&str>) -> Result<()> {
    use std::io::Write;
    ...
    match fs::symlink_metadata(&path) {
        Ok(metadata) if metadata.file_type().is_symlink() || !metadata.is_file() => return invalid(&path, "refusing to replace a non-regular state file"),
    ...
    let project_id = config.project_id.ok_or_else(|| Error::LocalEnvironment {
        path: config_path.to_path_buf(), reason: "project-id is required; run `pinset migrate` first".to_owned(),
    })?;
    let record = LocalEnvironment { schema: 1, root, project_id, profile: profile.to_owned() };
    ...
    validate_directory(directory)?;
    let mut file = atomic_write_file::AtomicWriteFile::open(&path).map_err(|error| local_error(&path, error))?;
    file.write_all(content.as_bytes()).and_then(|()| file.commit()).map_err(|error| local_error(&path, error))
}

fn local_error(path: &Path, error: impl std::fmt::Display) -> Error {
    Error::LocalEnvironment { path: path.to_path_buf(), reason: error.to_string() }
}

fn invalid<T>(path: &Path, reason: impl Into<String>) -> Result<T> {
    Err(Error::LocalEnvironment { path: path.to_path_buf(), reason: reason.into() })
}
    ...
    fn local_choices_are_bound_to_project_identity_and_ci_is_read_only() {
        ...
        config.environment = Some(ProjectEnvironment {
            ...
            profiles: ["dev", "test"].into_iter().map(|name| (name.into(), EnvironmentProfile {
                file: format!("pinset.env/{name}.age"), recipients: vec!["age1example".into()],
        ...
        assert_eq!(select_environment(&home, &config_path, &config, None, None, false).unwrap().source, "local");
        assert_eq!(select_environment(&home, &config_path, &config, None, None, true).unwrap().profile.as_deref(), Some("dev"));
        assert_eq!(select_environment(&home, &config_path, &config, Some("dev"), Some("test"), false).unwrap().source, "argument");
        assert_eq!(select_environment(&home, &config_path, &config, None, Some("dev"), false).unwrap().source, "process");
        ...
        assert_eq!(select_environment(&home, &config_path, &config, None, None, false).unwrap().source, "project");
    }
}
