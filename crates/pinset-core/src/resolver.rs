...
pub fn execution_context(tool: &str, executable: &Path, cwd: &Path, home: &Path) -> Result<ExecutionContext> {
    let environment = selected_runtime_environment(tool, cwd, home);
    let remove_environment = if environment.iter().any(|variable| variable.name == "VIRTUAL_ENV") {
        vec!["PYTHONHOME"]
    } else { Vec::new() };
    Ok(ExecutionContext {
...
pub fn resolve_execution_command(command: &str, cwd: &Path, home: &Path) -> Result<CommandResolution> {
    if command_tool(command).is_some() {
        return resolve_command(command, cwd, home);
    }
    let configured = effective_configured_tools(cwd, home)?;
    for provider in runtime_providers().iter().filter(|provider| configured.contains_key(provider.tool)) {
    ...
    if !explicit_path && configured.contains_key("python") {
        match resolve_project_python_command(command, cwd, home) {
            ...
            Err(Error::RuntimeCommandNotFound { .. } | Error::PythonEnvironmentSelectionMissing { .. }) => {},
    ...
    let executable = if explicit_path {
        let candidate = if command_path.is_absolute() { command_path.to_path_buf() } else { cwd.join(command_path) };
        let candidate = fs::canonicalize(&candidate).unwrap_or(candidate);
        (is_executable_file(&candidate) && !excluded.iter().any(|shim| same_executable(&candidate, shim, command)))
            .then_some(candidate)
    } else {
        find_system_commands(command, cwd, home, path.as_deref(), &excluded).into_iter().next()
    }.ok_or_else(|| Error::CommandSelectionNotFound {
        command: command.to_owned(), searched: path.map(|value| value.to_string_lossy().into_owned()).unwrap_or_default(),
    })?;
    Ok(CommandResolution {
        command: command.to_owned(), tool: String::new(), requested: None,
        version: "unknown".into(), source: SelectionSource::System,
        selection_path: find_project_context(cwd)?.config_path, executable,
    })
}
...
pub fn path_with_selected_tools(
    ...
    let mut entries = if tool.is_empty() { Vec::new() } else { vec![selected_dir.clone()] };
...
pub fn selected_runtime_environment(
    ...
    let mut providers = if tool.is_empty() { Vec::new() } else {
        let Ok(providers) = provider_dependency_order(tool) else { return variables; };
        providers
    };
