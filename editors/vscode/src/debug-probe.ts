...
export async function debugProbe(folder: vscode.WorkspaceFolder, runtime: RuntimeDescriptor, storage: vscode.Uri,
  ...
  const expected = language === "flutter"
    ...
    : runtime.executable;
  const source = language === "node" ? `console.log(${JSON.stringify(marker)}+JSON.stringify({executable:process.execPath,version:process.versions.node}));`
    : language === "python" ? `import sys,json\nprint(${JSON.stringify(marker)}+json.dumps(dict(executable=sys.executable,version='.'.join(map(str,sys.version_info[:3])))))\n`
    : `import 'dart:io'; import 'dart:convert'; void main() { print('${marker}' + jsonEncode({'executable': Platform.resolvedExecutable, 'version': Platform.version.split(' ').first})); }`;
  ...
  const config: vscode.DebugConfiguration = { name, type: language === "node" ? "node" : language === "python" ? "debugpy" : "dart",
    ...
    env: { NODE_OPTIONS: null, NODE_PATH: null, PYTHONPATH: null, PYTHONSTARTUP: null, PYTHONINSPECT: null }, envFile: "" };
  ...
  const termination = vscode.debug.onDidTerminateDebugSession(ended => { if (ended.name === name) resolveDone?.(); });
  ...
  try {
    ...
    const equal = await fs.realpath(expected).then(async expectedPath => expectedPath === await fs.realpath(observed!.executable)).catch(() => false);
    ...
    clearTimeout(timeout); tracker.dispose(); termination.dispose(); cancellation.dispose();
